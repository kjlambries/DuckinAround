...
@export var rate : float = 0.5 # rate of convergence to offset (higher = faster, units 1/seconds)
...
func _process(delta: float) -> void:
	...
	global_transform.basis = lerp (global_transform.basis,tbasis,delta/rate)
